...
- **Expected** — `Unexpected` allocates the error and its control block on Base's heap (`Heap::MakeShared`, or `Shared::MakePointer` for a derived error). The call stays `Unexpected<E>("… {}", args…)`. Formatting runs in the caller; `E` is constructed from the resulting string. `Unexpected(result.error())` forwards that pointer and does not allocate. `Expected<T, E>` is still `std::expected` of `shared_ptr<E>`. Covered by `ExpectedTests`.
