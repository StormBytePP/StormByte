...
- **WCString** — wide counterpart of `CString` (`wchar_t` / `std::wstring` / `std::wstring_view` / `std::wostream`). Same comparison, hash, subscript and lifetime rules as `std::wstring::c_str()`. Construct from `const wchar_t*` (null stays null), `std::wstring_view` and `const std::wstring&` (copy onto Base's heap; empty yields `L""`). Covered by `WCStringTests`.
