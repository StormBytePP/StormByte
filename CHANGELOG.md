...
- **Shared** — owning handle for an object allocated on Base's heap. Construct only with `Heap::MakeShared` or `Clonable::MakePointer`. Converts implicitly to `std::shared_ptr<T>` (same control block, deleter stays Base). No conversion from `std::shared_ptr`.
- **Unique** — unique owning handle for an object allocated on Base's heap. Construct only with `Heap::MakeUnique` or `Clonable::MakePointer`. Converts implicitly (move) to `std::unique_ptr<T, Heap::ObjectDeleter>`. No conversion from or to `std::unique_ptr<T>`.
