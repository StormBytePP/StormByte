...
- **Shared** — owning handle for an object allocated on Base's heap (`safe_pointers.hxx`). Exact type: `Heap::MakeShared`. Derived type: `Shared<Base>::MakePointer<Derived>` (the deleter still destroys the derived object). Converts implicitly to `std::shared_ptr<T>` (same control block, deleter stays Base). No conversion from `std::shared_ptr`. Covered by `SafePointersTests`.
- **Unique** — unique owning handle for an object allocated on Base's heap (`safe_pointers.hxx`). Exact type: `Heap::MakeUnique`. Derived type: `Unique<Base>::MakePointer<Derived>` (`Derived` needs a virtual destructor). Converts implicitly (move) to `std::unique_ptr<T, Heap::ObjectDeleter>`. No conversion from or to `std::unique_ptr<T>`. `Heap` stays private and is not installed. Covered by `SafePointersTests`.
