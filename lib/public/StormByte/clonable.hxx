...
namespace StormByte {
	 ...
	 *
	 * Construct only with @ref Heap::MakeShared or @ref Clonable::MakePointer.
	 * Converts implicitly to `std::shared_ptr<T>` (same control block).
	 * There is no conversion from `std::shared_ptr`.
	 */
	template<class T>
	class Shared {
		public:
	 ...
	 * @tparam T Pointee type.
	 *
	 * Construct only with @ref Heap::MakeUnique or @ref Clonable::MakePointer.
	 * Converts implicitly to `std::unique_ptr<T, Heap::ObjectDeleter>` on move.
	 * There is no conversion from or to `std::unique_ptr<T>`.
	 */
	template<class T>
	class Unique {
		public:
